        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="/Parent/View/StudentFeesDetails/{{$adm_no}}">{{ucwords(strtolower($student_data->first_name))}}`s Fees Details</a></li>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="row">
                        <div class="card">
                            <div class="card-body">
                                <a href="/Parent/View/StudentFeesDetails/{{$adm_no}}" class="btn btn-sm btn-secondary my-2"><i class="bi bi-arrow-left"></i> Back To Fees Lists</a>
                                <h5 class="card-title">Note:</h5>
                                <ul>
                                    <li>See the fees data.</li>
                                    <li>Incase any information is incorrect contact your school administrator.</li>
                                </ul>
                            </div>
                        </div>
                        <!-- Recent Sales -->
                        <div class="col-12">
                            <div class="card recent-sales overflow-auto">
                                <div class="card-body">
                                    ...
                                    <p class="my-1 text-dark row"><b class="col-md-3">Amount Paid: </b> <span class="col-md-9">Kes {{number_format($transaction->amount)}}</span></p>
                                    <p class="my-1 text-dark row"><b class="col-md-3">Date Paid: </b> <span class="col-md-9">{{date("D dS M Y",strtotime($transaction->date_of_transaction))}}</span></p>
                                    <p class="my-1 text-dark row"><b class="col-md-3">Payment Purpose: </b> <span class="col-md-9">{{$transaction->payment_for}}</span></p>
                                    <p class="my-1 text-dark row"><b class="col-md-3">Mode of Payment: </b> <span class="col-md-9">{{$transaction->mode_of_pay}}</span></p>
                                    <p class="my-1 text-dark row"><b class="col-md-3">Transaction Code: </b> <span class="col-md-9">{{$transaction->transaction_code}}</span></p>
                                    <p class="my-1 text-dark row"><b class="col-md-3">Paid By: </b> <span class="col-md-9">{{ucwords(strtolower($paid_by))}}</span></p>
                                    
                                    <hr class="my-2">
                                    <h5 class="card-title">Supporting Documents</h5>
                                    @if (isJson($transaction->support_document))
                                        @php
                                            $support_document = json_decode($transaction->support_document);
                                        @endphp
                                        @if (count($support_document) > 0)
                                            @for ($index=0; $index < count($support_document); $index++)
                                                <a href="https://lsims.ladybirdsmis.com{{$support_document[$index]->fileLocation}}" class="link" target="_blank"><b>{{$index+1}}. {{$support_document[$index]->fileName}} <i class="bi bi-box-arrow-up-right"></i></b></a>
                                                <iframe class="iframe_thumbnail" src="https://lsims.ladybirdsmis.com{{$support_document[$index]->fileLocation}}" frameborder="">
                                                </iframe>
                                            @endfor
                                        @else
                                            <p class="text-secondary">No supporting documents available {{$transaction->support_document}}.</p>
                                        @endif
                                    @else
                                        <p class="text-secondary">No supporting documents available {{$transaction->support_document}}.</p>
                                    @endif
