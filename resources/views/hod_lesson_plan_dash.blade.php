...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1>Lesson Plan <button class="btn btn-sm btn-primary">H.O.D Section</button></h1>
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">H.O.D Lesson Plan Manager</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="row">
                        <div class="card">
                            <div class="card-body">
                                <a href="/Teacher/LessonPlan" class="btn btn-secondary btn-sm my-2"><i class="bi bi-arrow-left"></i> Back to Subjects I teach</a>
                                <h5 class="card-title">Note:</h5>
                                <ul>
                                    <li>You can only Manage the subjects that are in your departments you head.</li>
                                    <li>If none of the subjects you teach appear here contact your Administrator.</li>
                                    <li>Click on the class you want to manage your lesson plan.</li>
                                </ul>
                            </div>
                        </div>
                        @php
                        ...
                        <div class="card recent-sales overflow-auto">
                            <div class="card-body">
                                <h5 class="card-title">Subjects I Teach <span></h5>
                                ...
                                <table class="table table-borderless datatable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col">Classes Taught</th>
                                            {{-- <th scope="col">Status</th> --}}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @for ($i = 0; $i < count($subjects_to_monitor); $i++)
                                            <tr>
                                                <th scope='row'><a href='#'>{{ $i + 1 }}</a></th>
                                                <td style="min-width: 100px;">{{ $subjects_to_monitor[$i]->subject_name }}</td>
                                                <td style="min-width: 100px;">{{ $subjects_to_monitor[$i]->display_name }}</td>
                                                <td style="min-width: 100px;">{{ $subjects_to_monitor[$i]->timetable_id }}</td>
                                                <td style="min-width: 100px;">
                                                    @for ($i_1 = 0; $i_1 < count($subjects_to_monitor[$i]->classes_taught); $i_1++)
                                                        @if ($i_1 + 1 == count($subjects_to_monitor[$i]->classes_taught))
                                                            <a class="my-2" href='/Teacher/HOD/Create/Lessonplan/{{$subjects_to_monitor[$i]->subject_id}}/Class/{{$subjects_to_monitor[$i]->classes_taught[$i_1]}}' data-bs-toggle="tooltip" data-bs-placement="top" title="Click Me to Manage the lesson plans for {{ $subjects_to_monitor[$i]->display_name }}, {{ classNameAdms($subjects_to_monitor[$i]->classes_taught[$i_1]) }}">{{ classNameAdms($subjects_to_monitor[$i]->classes_taught[$i_1]) }}</a>
                                                        @else
                                                            <a class="my-2" href='/Teacher/HOD/Create/Lessonplan/{{$subjects_to_monitor[$i]->subject_id}}/Class/{{$subjects_to_monitor[$i]->classes_taught[$i_1]}}' data-bs-toggle="tooltip" data-bs-placement="top" title="Click Me to Manage the lesson plans for {{ $subjects_to_monitor[$i]->display_name }}, {{ classNameAdms($subjects_to_monitor[$i]->classes_taught[$i_1]) }}">{{ classNameAdms($subjects_to_monitor[$i]->classes_taught[$i_1]) }}</a> || 
                                                        @endif
                                                    @endfor
                                                </td>
                                                {{-- <td style="min-width: 100px;"><span class='badge bg-success'>Week 5</span></td> --}}
                                            </tr>
                                        @endfor
                                    </tbody>
                                </table>
