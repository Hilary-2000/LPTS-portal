        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"> Message & Alerts</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                ...
                <div class="col-lg-12">
                    <div class="row">
                        ...
                        <div class="card recent-sales overflow-auto">
                            <div class="card-body">
                                <a href="/Teacher/Messages/CreateAlert" class="btn btn-secondary btn-sm my-2"><i class="bi bi-arrow-left"></i> Create Message & Alerts</a>
                                {{-- create notice only if you are the administrator or the headteacher deputy headteacher --}}
                                <h5 class="card-title"> Message & Alerts</h5>
                                @php
                                ...
                                <table class="table table-borderless datatable">
                                    ...
                                    <tbody>
                                        @for ($i = 0; $i < count($message_notification); $i++)
                                            <tr class="">
                                                <td><b>{{$i+1}}</b></td>
                                                <td>{{strlen($message_notification[$i]->message_title) > 200 ? substr($message_notification[$i]->message_title,0,200)."..." : $message_notification[$i]->message_title}} <br>
                                                    <span class="text-primary">{{"{".ucwords(strtolower($message_notification[$i]->owner_type))."}"}}</span>
                                                </td>
                                                <td>{{strlen(getInnerText($message_notification[$i]->message_body)) > 200 ? substr(getInnerText($message_notification[$i]->message_body),0,200)."..." : getInnerText($message_notification[$i]->message_body)}}</td>
                                                <td>{{date("D dS M Y @ h:i:sA",strtotime($message_notification[$i]->date_created))}}</td>
                                                <td>
                                                    @if ($message_notification[$i]->message_edit_status == "Drafted")
                                                        <span class="badge bg-secondary">Drafted</span>
                                                    @else
                                                        <span class="badge bg-success">Published</span>
                                                    @endif
                                                </td>
                                                <td style="min-width: 100px;"><a href="/Teacher/Messages/Manage/{{$message_notification[$i]->message_editor_id}}" class="btn btn-link btn-sm p-1"><i class="bi bi-gear"></i> Manage</a></td>
                                            </tr>
                                        @endfor
                                    </tbody>
                                </table>
