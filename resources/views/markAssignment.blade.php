...
<head>
    ...
    <title>Assignments - {{$subject_name}} -
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="/Teacher/Assignment">Assignments - {{$subject_name}} - {{$selected_class}}</a>
                    </li>
                    <li class="breadcrumb-item active">Mark Assignment - <b>"{{$assignment->name}}"</b></li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="row">
                        <div class="card">
                            <div class="card-body">
                                <a href="/Teacher/Assignments/{{$subject_id}}/Create/{{$class_name}}" class="btn btn-secondary btn-sm my-2"><i class="bi bi-arrow-left"></i> Back to Assignments</a>
                                <h5 class="card-title">Note:</h5>
                                <ul>
                                    <li>Mark assignments from this page.</li>
                                </ul>
                            </div>
                        </div>
                        <!-- Recent Sales -->
                        <div class="col-12">
                            <div class="card recent-sales overflow-auto">
                                <div class="card-body">
                                    <h5 class="card-title">Mark Assignment - "{{$assignment->name}}"</h5>
                                    ...
                                    <table class="table table-borderless datatable">
                                        ...
                                        <tbody>
                                            @php
                                                $student_ans = $assignment->answers;
                                            @endphp
                                            @if (isJson($student_ans))
                                                @php
                                                    $student_ans = json_decode($student_ans);
                                                @endphp

                                                {{-- student lists --}}
                                                @for ($i = 0; $i < count($student_ans); $i++)
                                                    <tr>
                                                        <th scope="row"><a href="#">1</a></th>
                                                        <td style="min-width: 100px;">{{$student_ans[$i]->student_name}}</td>
                                                        <td style="min-width: 100px;">{{date("D dS M Y",strtotime($student_ans[$i]->date_completed))}}</td>
                                                        <td style="min-width: 100px;">
                                                            @if ($student_ans[$i]->marked)
                                                                <span class="badge bg-success">Marked</span>
                                                            @else
                                                                <span class="badge bg-secondary">Not Marked</span>
                                                            @endif
                                                        </td>
                                                        <td style="min-width: 100px;">{{$student_ans[$i]->marks_attained}} Mks</td>
                                                        <td style="min-width: 100px;"><a href="/Teacher/Assignment/Mark/{{$assignment_id}}/{{$student_ans[$i]->student_id}}" class="btn btn-success btn-sm w-100 my-2"><i class="bi bi-check"></i> Mark</a></td>
                                                    </tr>
                                                @endfor
                                            @else
                                                @for ($i = 0; $i < count($student_ans); $i++)
