        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Subjects I teach</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                    ...
                    <div class="row">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <ul>
                                    <li>You can only create a plan for the subjects you teach.</li>
                                    <li>If none of the subjects you teach appear here contact your Administrator.</li>
                                    <li>Click on the class you want to edit your lesson plan.</li>
                                </ul>
                                @if (count($is_hod) > 0)
                                <h6 class="card-title">H.O.D Lesson PLan Manager</h6>
                                    @php
                                        $display_data = "";
                                        for ($index=0; $index < count($is_hod); $index++) { 
                                            $display_data.=$is_hod[$index]->name.", ";
                                        }
                                        $display_data = substr($display_data,0,strlen($display_data)-2);
                                    @endphp
                                    <ul>
                                        <li>You are selected as the H.O.D for {{$display_data}}</li>
                                    </ul>
                                    <a href="/Teacher/HOD/LessonPlan" class="btn btn-primary btn-sm"><i class="bi bi-kanban"></i> H.O.D Lesson Plan Management</a>
                                @endif
                            </div>
                        </div>
                        <!-- Recent Sales -->
                        <div class="card recent-sales overflow-auto">
                            <div class="card-body">
                                ...
                                <table class="table table-borderless datatable">
                                    ...
                                    <tbody>
                                        @for ($i = 0; $i < count($subjects_taught); $i++)
                                            <tr>
                                                <th scope='row'><a href='#'>{{ $i + 1 }}</a></th>
                                                <td style="min-width: 100px;">{{ $subjects_taught[$i]->subject_name }}</td>
                                                <td style="min-width: 100px;">{{ $subjects_taught[$i]->display_name }}</td>
                                                <td style="min-width: 100px;">{{ $subjects_taught[$i]->timetable_id }}</td>
                                                <td style="min-width: 100px;">
                                                    @for ($i_1 = 0; $i_1 < count($subjects_taught[$i]->class_taught); $i_1++)
                                                        @if ($i_1 + 1 == count($subjects_taught[$i]->class_taught))
                                                            <a class="my-2" href='/Teacher/Create/Lessonplan/{{$subjects_taught[$i]->subject_id}}/Class/{{$subjects_taught[$i]->real_class_names[$i_1]}}' data-bs-toggle="tooltip" data-bs-placement="top" title="Click Me to edit lesson plans for {{ $subjects_taught[$i]->display_name }}, {{ $subjects_taught[$i]->class_taught[$i_1] }}">{{ $subjects_taught[$i]->class_taught[$i_1] }}</a>
                                                        @else
                                                            <a class="my-2" href='/Teacher/Create/Lessonplan/{{$subjects_taught[$i]->subject_id}}/Class/{{$subjects_taught[$i]->real_class_names[$i_1]}}' data-bs-toggle="tooltip" data-bs-placement="top" title="Click Me to edit lesson plans for {{ $subjects_taught[$i]->display_name }}, {{ $subjects_taught[$i]->class_taught[$i_1] }}">{{ $subjects_taught[$i]->class_taught[$i_1] }}</a> : 
                                                        @endif
                                                    @endfor
